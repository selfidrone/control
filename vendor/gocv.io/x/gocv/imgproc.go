...
func ApproxPolyDP(curve []image.Point, epsilon float64, closed bool) (approxCurve []image.Point) {
	...
	cApproxCurvePoints := (*[1 << POINTER_SIZE]C.Point)(unsafe.Pointer(cApproxCurve.points))[:cApproxCurve.length:cApproxCurve.length]
...
func FillPoly(img *Mat, pts [][]image.Point, c color.RGBA) {
	...
	for i, pt := range pts {
		...
		pa := (*[1 << POINTER_SIZE]C.struct_Point)(unsafe.Pointer(p))
